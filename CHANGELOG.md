# Change Log

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [Released]

## [1.8.0] - 2023-12-12

### Added

- Show the import when listing delivery notes.

## [1.7.0] - 2023-11-26

### Added

- Allow filtering by anything when listing invoices and delivery notes.

## [1.6.0] - 2023-11-19

### Added

- Sync price with delivery note.

## [1.5.1] - 2023-11-19

### Fixed

- Price is not rounded to 2 decimal places on invoices.

## [1.5.0] - 2023-11-12

### Added

- Redesign filtering by farmer, trader and product on delivery notes and invoices.

## [1.4.0] - 2023-11-04

### Added

- Show the calculated net weight per weighing when generating a delivery note.

## [1.3.0] - 2023-10-15

### Added

- Allow previewing a delivery note before generating it.

## [1.2.0] - 2023-10-15

### Added

- Allow printing a delivery note before generating it.

## [1.1.1] - 2023-09-24

### Fixed

- Allow association between customer and template.

## [1.1.0] - 2023-09-22

### Added

- Rename supplier to trader.

## [1.0.0] - 2023-09-03

- Major release with all added features on v0.

## [Unreleased]

## [0.64.0] - 2023-09-03

### Added

- Establish pagination for invoices.

## [0.63.0] - 2023-08-25

### Added

- Added feature to save the total weight on invoices.

## [0.62.0] - 2023-08-20

### Added

- Added feature to check a password.

## [0.61.0] - 2023-08-20

### Added

- Added feature to establish a password.

## [0.60.0] - 2023-08-19

### Changed

- Only prints 1 copy per invoice.

## [0.59.0] - 2023-08-15

### Added

- Migrate code from Java 11 to Java 8.

## [0.58.0] - 2023-08-14

### Added

- Added feature to close an invoice.

## [0.57.0] - 2023-08-13

### Added

- Added feature to determine commissions.

## [0.56.0] - 2023-08-13

### Added

- Added feature to list and delete invoices.

## [0.55.0] - 2023-08-06

### Added

- Added feature to generate an invoice.

## [0.54.1] - 2023-07-08

### Fixed

- Fixed an error where the total net weight and boxes quantity are not updated when a delivery note is removed.

## [0.54.0] - 2023-07-06

### Added

- Establish relationship between farmer and supplier customers.

## [0.53.0] - 2023-07-02

### Added

- Added feature to delete a delivery note.

## [0.52.0] - 2023-06-27

### Added

- Added feature to list delivery notes.

## [0.51.0] - 2023-06-09

### Added

- Added feature to indicate whether a customer is a supplier or not.

## [0.50.0] - 2023-06-07

### Added

- Added feature to save a delivery note.

## [0.49.0] - 2023-06-03

### Added

- Added feature to generate a delivery note.

## [0.48.0] - 2023-05-21

### Added

- Add a weighings table.

## [0.47.0] - 2023-05-20

### Added

- Add distinction between pallet and box on the container entity.

## [0.46.0] - 2023-05-19

### Removed

- Removed the price from the product.
>>>>>>> origin/main

## [0.45.0] - 2023-05-07

### Added

- Added feature to remove a delivery note item.

## [0.44.0] - 2023-05-07

### Added

- Added feature to edit a delivery note item.

## [0.43.0] - 2023-05-01

### Added

- Added feature to add a delivery note item.

## [0.42.0] - 2023-04-21

### Added

- Added feature to restore a container.

## [0.41.0] - 2023-04-21

### Added

- Added feature to remove a container.

## [0.40.0] - 2023-04-20

### Added

- Added feature to update a container.

## [0.39.0] - 2023-04-20

### Added

- Added feature to list containers.

## [0.38.0] - 2023-04-19

### Added

- Added feature to register a container.

## [0.37.0] - 2023-04-18

### Added

- Added feature to restore a product.

## [0.36.0] - 2023-04-18

### Added

- Added feature to remove a product.

## [0.35.0] - 2023-04-18

### Added

- Added feature to update the product data.

## [0.34.0] - 2023-04-15

### Added

- Added feature to list products.

## [0.33.0] - 2023-04-12

### Added

- Added feature to register a product.

## [0.32.0] - 2023-04-06

### Removed

- Invoice item entity, as its concept is going to change.

## [0.31.0] - 2023-04-06

### Added

- Restriction not to allow ranges on spreadsheet positions from template fields.

## [0.30.1] - 2023-02-17

### Fixed

- Fixed an issue where there could be two customers with the same TIN.

## [0.30.0] - 2023-02-17

### Added

- Added feature to remove an invoice item.

## [0.29.0] - 2023-02-16

### Added

- Added feature to edit an invoice item.

## [0.28.0] - 2023-02-16

### Added

- Added feature to add an invoice item.

## [0.27.1] - 2023-02-11

### Fixed

- Fixed an issue where removed variables could be edited.

## [0.27.0] - 2023-02-11

### Added

- Added feature to restore a template.

## [0.26.0] - 2023-02-11

### Added

- Added feature to remove a template.

## [0.25.0] - 2023-02-09

### Added

- Added feature to edit a template.

## [0.24.0] - 2023-02-08

### Added

- Added feature to show a template.

## [0.23.0] - 2023-02-02

### Added

- Added feature to list templates.

## [0.22.0] - 2023-01-30

### Added

- Added feature to register a template.

## [0.21.0] - 2023-01-26

### Added

- Added feature to edit a template field.

## [0.20.0] - 2023-01-26

### Added

- Added feature to remove a template field.

## [0.19.0] - 2023-01-25

### Added

- Added feature to add a template field.

## [0.18.0] - 2023-01-21

### Added

- Added feature to restore a subtotal.

## [0.17.3] - 2023-01-21

### Fixed

- Fixed an issue where a variable with a removed subtotal could be restored.

## [0.17.2] - 2023-01-20

### Fixed

- Fixed an issue where a subtotal could not be removed if it was associated to a removed variable.

## [0.17.1] - 2023-01-20

### Fixed

- Fixed an issue where a removed subtotal could be associated to a variable.

## [0.17.0] - 2023-01-20

### Added

- Added feature to remove a subtotal.

## [0.16.0] - 2023-01-18

### Added

- Added feature to restore a variable.

## [0.15.1] - 2023-01-18

### Fixed

- Fixed an issue where a removed variable could be edited.

## [0.15.0] - 2023-01-18

### Added

- Added feature to remove a variable.

## [0.14.0] - 2023-01-16

### Added

- Added feature to edit a variable.

## [0.13.0] - 2023-01-14

### Added

- Added feature to list variables.

## [0.12.0] - 2023-01-12

### Added

- Added feature to add a variable.

## [0.11.0] - 2023-01-08

### Added

- Added feature to edit a subtotal.

## [0.10.0] - 2023-01-08

### Added

- Added feature to list subtotals.

## [0.9.0] - 2023-01-06

### Added

- Added feature to calculate a subtotal.

## [0.8.0] - 2023-01-06

### Added

- Added feature to add a subtotal.

## [0.7.0] - 2023-01-05

### Added

- Added feature to update the customer data.

## [0.6.0] - 2023-01-03

### Added

- Added feature to restore a customer.

## [0.5.0] - 2023-01-03

### Added

- Added feature to remove a customer.

## [0.4.0] - 2022-12-30

### Added

- Added feature to show the details of a customer.

## [0.3.0] - 2022-12-28

### Added

- Added feature to list customers.

## [0.2.0] - 2022-12-25

### Added

- Added feature to register a customer.

## [0.1.0] - 2022-12-24

### Added

- Add a modal to show the information about the project.
- Configured the main project frame.
- Add localization to the user interface to support different languages.
- Set up the application configuration (version, application name and project URL).
